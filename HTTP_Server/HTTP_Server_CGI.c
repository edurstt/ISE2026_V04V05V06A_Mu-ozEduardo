...
void netCGI_ProcessData (uint8_t code, const char *data, uint32_t len) {
  ...
  passw[0] = 1;
  do {
    /* GET DE LOS VALORES DE LOS LEDS */
    ...
    if (var[0] != 0) {
      ...
      if (strcmp (var, "led0=on") == 0) {
        ...
        if (netHTTPs_LoginActive()) {
		  ...
		  /* GET DE LOS VALORES DE LOS LCDs */
      else if (strncmp (var, "lcd1=", 5) == 0) { // Compara los 5 primeros caracteres, si es asi:
        // LCD Module line 1 text
        strcpy (lcd_text[0], var+5); 						 // le asigna el valor menos los 5 primeros
        osThreadFlagsSet (TID_Display, 0x01U); // ---> DESDE AQUI SE ENVIA LA SEÑAL QUE RECIBE LA FUNCION DISPLAY DEL HTTP_Server.
      }
        ...
        osThreadFlagsSet (TID_Display, 0x01U); // ---> DESDE AQUI SE ENVIA LA SEÑAL QUE RECIBE LA FUNCION DISPLAY DEL HTTP_Server.
			 ...
			 /* FIN GET DE LOS VALORES DE LOS LCDs */
    }
  } while (data);
    LED_SetOut_stm(P2); // endender o apagar el led P2 ya asignado
}
